...
const ClassManagement = () => {
  ...
  const handleUpdateClass = () => {
    ...
    const updatedClasses = classes.map((classItem) =>
      classItem.id === currentClass.id ? { ...classItem, name: newClassName } : classItem
    );
  ...
  return (
    ...
    <div className="p-4 md:p-8">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">Class Management</h2>

      {/* Add/Edit Class Section */}
      <div className="mb-6">
        <h3 className="text-xl font-semibold text-gray-700 mb-2">{editMode ? 'Edit Class' : 'Add New Class'}</h3>
        <div className="flex items-center space-x-4">
          ...
          <button
            className={`${
              editMode ? 'bg-yellow-600 hover:bg-yellow-700' : 'bg-blue-600 hover:bg-blue-700'
            } text-white p-2 rounded-md transition-colors`}
            onClick={editMode ? handleUpdateClass : handleAddClass}
          >
            {editMode ? <FaEdit className="inline mr-2" /> : <FaPlus className="inline mr-2" />}
            {editMode ? 'Update' : 'Add'}
          </button>
        </div>
      </div>
      ...
      {/* Classes List */}
      <div>
        <h3 className="text-xl font-semibold text-gray-700 mb-2">Classes List</h3>
        <table className="min-w-full bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <tbody>
            {classes.map((classItem) => (
              <tr key={classItem.id} className="border-b border-gray-200">
                <td className="py-3 px-4">{classItem.id}</td>
